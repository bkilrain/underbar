(function() {
    ...
    _.reject = function(collection, test) {
        ...
        var testPassed = _.filter(collection, test);
        return _.filter(collection, function(x) {
            return _.indexOf(testPassed, x) < 0;
        });
    ...
    _.map = function(collection, iterator) {
        ...
        _.each(collection, function(item, index) {
            mappedArray.push(iterator(item, index, collection));
        });
        return mappedArray;
    };
    ...
    _.memoize = function(func) {
        ...
        return function() {
            ...
            if (!calledArgs[currentArgs] || calledArgs[currentArgs].argsLength !== arguments.length) { // hacky!
                calledArgs[currentArgs] = {
                    "result": func.apply(this, arguments),
                    "argsLength": arguments.length
                };
            }
            return calledArgs[currentArgs].result;
        }
    ...
    _.shuffle = function(array) {
        var shuffleIt = array.slice();
        var shuffled = [];
        var randomIndex = function(max) {
            return Math.floor(Math.random() * (max));
        }
        while (shuffleIt.length > 0) {
            shuffled.push(shuffleIt.splice(randomIndex(shuffleIt.length), 1)[0]);
        }
        return shuffled;
    };
    ...
    _.zip = function() {
        var zipped = [];
        var args = Array.prototype.slice.call(arguments);
        _.each(args, function(argsArray, argsArrayIndex) {
            for (var i = 0; i < args.length; i++) {
                if (argsArrayIndex === 0) {
                    zipped.push([]);
                }
                zipped[i].push(argsArray[i]);
            }
        });
        return zipped;
    };
    ...
    _.intersection = function() {
        var args = Array.prototype.slice.call(arguments);
        var intersected = args[0].slice();
        _.each(args, function(argsArray, index) {
            if (index > 0) {
                var item = argsArray[i];
                for (var i = 0; i < argsArray.length; i++) {
                    if (!(_.contains(intersected, item))) {
                        intersected.splice(_.indexOf(intersected, item), 1);
                    }
                }
            }
        });
    ...
    _.difference = function(array) {
        var args = Array.prototype.slice.call(arguments);
        var different = args[0].slice();
        _.each(args, function(argsArray, index) {
            if (index > 0) {
                for (var i = 0; i < argsArray.length; i++) {
                    var item = argsArray[i];
                    if (_.contains(different, item)) {
                        different.splice(_.indexOf(different, item), 1);
                    }
                }
            }
        });
    ...
    _.throttle = function(func, wait) {
        ...
        return function() {
            if (throttled === 'no') {
                latestVal = func.call(null);
                throttled = 'yes';

              //timer function runs depending on number of consecutive throttled calls
                var timer = setTimeout(function() {
                    if (throttled === 'yes') { // if no more calls, throttle gate opens after wait
                        throttled = 'no';
                    } else if (throttled === 'waiting') { // if more calls, throttled function invoked after wait
                        latestVal = func.call(null);
                    }
                }, wait);
            } else if (throttled === 'yes') { // sets the function to automatically call again after wait
                throttled = 'waiting';
            }
